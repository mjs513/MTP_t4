...
const int csd[] = {0xff, 9};
#else
const char *sd_str[] = {"sd1"}; // edit to reflect your configuration
const int cs[] = {8}; // edit to reflect your configuration
const int csd[] = {9}
#endif
...
void setup()
{
  ...
  for(int ii=0; ii<nsd; ii++)
  {
    ...
    {
      SDMTPClass sdx[nsd](mtpd, storage, sd_str[ii], cs[ii]);
    } else {
      SDMTPClass sdx[nsd](mtpd, storage, sd_str[ii], cs[ii], csd[ii], SHARED_SPI, SPI_SPEED);
    }
    if (!sdx[nsd].init(true)) { // init the object and add it to the list
  ...
  if ( Serial.available() ) {
    ...
    switch (command) {
      ...
      case 's':
      {
        ...
        // to the file created.
        dataFile = sdx[drive_index].open("datalog.txt", FILE_WRITE);
        logData();
      }
      break;
...
void dumpLog()
{
  Serial.println("\nDumping Log!!!");
  // open the file.
  dataFile = sdx[drive_index].open("datalog.txt");
...
{
  Serial.print("\n Space Used = ");
  Serial.println(sdx[drive_index].usedSize());
  Serial.print("Filesystem Size = ");
  Serial.println(sdx[drive_index].totalSize());

  printDirectory(sdx[drive_index]);
}
...
void eraseFiles()
{
  ...
  PFsVolume partVol;
  if (!partVol.begin(sdx[drive_index].sdfs.card(), true, 1)) {
