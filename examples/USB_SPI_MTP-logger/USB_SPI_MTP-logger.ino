...
void storage_configure()
{
  ...
  if (lfsram.begin (LFSRAM_SIZE)) {
    ...
    lfsmtpcb.set_formatLevel(true);  //sets formating to lowLevelFormat, false indicates quickFormat
...
//Configure NAND SPI Flash chip
   pinMode(nandCS,OUTPUT); digitalWriteFast(nandCS,HIGH);
   if(!lfsNAND.begin(nandCS, SPI))
    { Serial.printf("NAND SPIFlash Storage %d %d %s failed or missing", 0 ,nandCS,"NAND7"); Serial.println();
    }
    else
    {
